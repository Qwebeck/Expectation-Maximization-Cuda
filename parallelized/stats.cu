...
__global__ void subtract_row_vector(float *dev, float *x, float *mean, int n_row, int n_col)
{
...
__global__ void log_probability_map(float *x, int n_col, int rank, float log_pdet)
{
...
vector *calculate_log_pdf(matrix *x, vector *mean, matrix *U, float log_pdet, int rank)
{
    ...
    float *d_x;
    cudaMalloc(&d_x, x->n_row * x->n_col * sizeof(float));
    CHECK_CUDA_ERROR(cudaMemcpy(d_x, DATA(x), x->n_row * x->n_col * sizeof(float), cudaMemcpyHostToDevice));

    matrix *dev = matrix_zeros(x->n_row, x->n_col);
    float *d_dev;
    cudaMalloc(&d_dev, dev->n_row * dev->n_col * sizeof(float));
    // CHECK_CUDA_ERROR(cudaMemcpy(d_dev, DATA(dev), dev->n_row * dev->n_col * sizeof(float), cudaMemcpyHostToDevice));

    float *d_mean;
    cudaMalloc(&d_mean, mean->length * sizeof(float));
    CHECK_CUDA_ERROR(cudaMemcpy(d_mean, DATA(mean), mean->length * sizeof(float), cudaMemcpyHostToDevice));

    dim3 dimDevBlock(mean->length, 2);
    ...
    float *d_U;
    cudaMalloc(&d_U, U->n_row * U->n_col * sizeof(float));
    CHECK_CUDA_ERROR(cudaMemcpy(d_U, DATA(U), U->n_row * U->n_col * sizeof(float), cudaMemcpyHostToDevice));

    matrix *mahalanobis_distance = matrix_zeros(dev->n_row, U->n_col);
    float *d_mahalanobis_distance;
    CHECK_CUDA_ERROR(cudaMalloc(&d_mahalanobis_distance, mahalanobis_distance->n_row * mahalanobis_distance->n_col * sizeof(float)));
    // cudaMemcpy(d_mahalanobis_distance, DATA(mahalanobis_distance), mahalanobis_distance->n_row * mahalanobis_distance->n_col * sizeof(float), cudaMemcpyHostToDevice);
    ...
    float *d_result;
    cudaMalloc(&d_result, result->length * sizeof(float));
    // CHECK_CUDA_ERROR(cudaMemcpy(d_result, DATA(result), result->length * sizeof(float), cudaMemcpyHostToDevice));
    ...
    CHECK_CUDA_ERROR(cudaMemcpy(DATA(result), d_result, result->length * sizeof(float), cudaMemcpyDeviceToHost));
