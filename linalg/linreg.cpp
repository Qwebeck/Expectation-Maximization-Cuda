...
{
    struct linreg *lr = (linreg *)malloc(sizeof(struct linreg));
    return lr;
}
...
struct linreg *linreg_fit(struct matrix *X, struct vector *y)
{
    ...
    float norm_factor = 1 / (lr->n - lr->p - 1);
    float sigma_resid = sqrt(norm_factor * sigma_resid_sq);
    lr->y_hat = y_hat;
