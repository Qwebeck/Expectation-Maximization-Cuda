...
struct matrix *matrix_new(int n_row, int n_col)
{
    ...
    DATA(new_matrix) = (float *)malloc((sizeof(float)) * n_row * n_col);
...
struct vector *matrix_row_view(struct matrix *M, int row)
{
    ...
    float *row_p = DATA(M) + (row * M->n_col);
...
*/
struct matrix *matrix_M_minus_lambda_I(struct matrix *M, float lambda)
{
...
bool matrix_equal(struct matrix *M1, struct matrix *M2, float tol)
{
...
bool matrix_is_upper_triangular(struct matrix *M, float tol)
{
...
{
    struct qr_decomp *qr = (qr_decomp *)malloc(sizeof(struct qr_decomp));
    return qr;
}
