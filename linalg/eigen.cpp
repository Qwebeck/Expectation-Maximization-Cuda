...
{
  struct eigen *e = (eigen *)malloc(sizeof(struct eigen));
  return e;
}
...
struct eigen *eigen_solve(struct matrix *M, float tol, int max_iter)
{
  ...
  // MatrixXd m(M->n_row, M->n_col);
  // for (int i = 0; i < M->n_row; i++)
  // {
  //   for (int j = 0; j < M->n_col; j++)
  //   {
  //     m(i, j) = MATRIX_IDX_INTO(M, i, j);
  //   }
  // }
  ...
  // {
  //   VECTOR_IDX_INTO(eigenvalues, i) = eigenvalueMatrix(i, 0).real();
  // }
  // eigen_solve_eigenvalues(M, tol, max_iter);
  struct matrix *eigenvectors = eigen_solve_eigenvectors(M, eigenvalues, tol, max_iter);
  // matrix_new(M->n_col, M->n_col);
  // for (int i = 0; i < M->n_col; i++)
  // {
  //   for (int j = 0; j < M->n_col; j++)
  //   {
  //     MATRIX_IDX_INTO(eigenvectors, i, j) = eigenvectorMatrix(i, j).real();
  //   }
  // }
    ...
    struct matrix *M, float eigenvalue, float tol, int max_iter)
{
  ...
  float lambda = eigenvalue + ((float)rand() / (float)RAND_MAX) * 0.000001;
