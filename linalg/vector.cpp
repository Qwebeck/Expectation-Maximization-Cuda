...
struct vector *vector_new(int length)
{
    ...
    struct vector *new_vector = (vector *)malloc(sizeof(struct vector));
    check_memory((void *)new_vector);

    DATA(new_vector) = (float *)malloc((sizeof(float)) * length);
...
*/
struct vector *vector_new_view(struct linalg_obj *parent, float *view, int length)
{
    assert(length >= 0);
...
struct vector *vector_from_array(float *data, int length)
{
    assert(length > 0);
...
struct vector *vector_constant(int length, float x)
{
    assert(length > 0);
    struct vector *v = vector_new(length);
    for (int i = 0; i < v->length; i++)
...
/* Construct a vector of equally spaced points within the closed
   interval [min, max].
*/
struct vector *vector_linspace(int length, float min, float max)
{
    assert(min <= max);
    assert(length > 1);
    struct vector *v = vector_new(length);
...
struct vector *vector_slice(struct vector *v, int begin_idx, int end_idx)
{
    ...
    float *begin_ptr = DATA(v) + begin_idx;
...
struct vector *vector_scalar_multiply(struct vector *v, float s)
{
    struct vector *w = vector_new(v->length);
...
void vector_scalar_multiply_into(struct vector *reciever, struct vector *v, float s)
{
    for (int i = 0; i < v->length; i++)
    {
...
bool vector_equal(struct vector *v1, struct vector *v2, float tol)
{
...
float vector_dot_product(struct vector *v1, struct vector *v2)
{
    assert(vector_lengths_equal(v1, v2));
    float dp = 0;
    for (int i = 0; i < v1->length; i++)
...
float vector_norm(struct vector *v)
{
    float norm_squared = vector_dot_product(v, v);
